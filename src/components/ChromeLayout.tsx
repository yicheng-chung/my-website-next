...
// Only these routes show the profile/music/reading column inline.
// Everywhere else it collapses into a drawer so the page content can use
// the full width.
const EXPANDED_ROUTES = ["/", "/about"];
...
export default function ChromeLayout({ children }: { children: ReactNode }) {
  ...
  const t = TEXT[lang];
  const isExpanded = EXPANDED_ROUTES.includes(pathname);
      ...
      {!isExpanded && (
        <>
          <button
            ...
            aria-label={t.open}
            className={`fixed top-1/2 left-0 z-30 flex h-12 w-7 -translate-y-1/2 items-center justify-center rounded-r-lg border border-l-0 border-neutral-200 bg-white/90 text-neutral-500 backdrop-blur-md transition-opacity hover:text-[#577E89] dark:border-neutral-700 dark:bg-neutral-800/90 dark:text-neutral-400 dark:hover:text-[#9BB8C2] ${
