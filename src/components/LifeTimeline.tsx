...
export default function LifeTimeline() {
  ...
  const [activeId, setActiveId] = useState<string | null>(timelineEvents[0]?.id ?? null);
  ...
  const scrollByArrow = (direction: 1 | -1) => {
    stripRef.current?.scrollBy({ left: direction * ARROW_SCROLL_AMOUNT, behavior: "smooth" });
  };
                ...
                {timelineEvents.map((event) => {
                  ...
                  return (
                    <button
                      ...
                      className="group relative z-10 flex shrink-0 cursor-pointer snap-center flex-col items-center gap-2 py-2"
                      style={{ width: NODE_WIDTH }}
                    >
                      <span
                        className={`rounded-full border-[3px] transition-all ${
                          isActive
                            ? "h-6 w-6 border-[#577E89] bg-[#577E89] dark:border-[#9BB8C2] dark:bg-[#9BB8C2]"
                            : "h-5 w-5 border-neutral-400 bg-white group-hover:scale-110 group-hover:border-[#577E89] dark:border-neutral-500 dark:bg-neutral-800 dark:group-hover:border-[#9BB8C2]"
                        }`}
                      />
                      <span
                ...
                >
                  <span
                    className={`h-3 w-3 flex-shrink-0 rounded-full ${
                      isOpen ? "bg-[#577E89] dark:bg-[#9BB8C2]" : "bg-neutral-300 dark:bg-neutral-600"
                    }`}
                  />
